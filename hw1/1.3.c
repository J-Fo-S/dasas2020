...
#define NUM_COMMANDS 11 // for gnuplot pipe command
...
{    
    ...
    for (i=0; i<h_t; ++i)
    {
        ...
        for (k=0; k<m_fft; ++k)
        {
                ...
                for (j=0; j<m_fft; ++j)
                {
                    ...
                    X_r[i][k] += Y[i * m_fft+j] * G[j] * cos((j * k * 2.0 * M_PI) / m_fft); // not sure about j term (i.e. relationship to n in DFT)
                    X_i[i][k] -= Y[i * m_fft+j] * G[j] * sin((j * k * 2.0 * M_PI) / m_fft);
                    //printf("%lf \n", Y[i*m_fft+j]);
    ...
    "set title \"STFT\"", 
    "set xlabel 'number of stft frames (time/ stft points)'",
    "set ylabel 'freq HZ'",
    ...
    "set xrange [1:63]",
    "set yrange [1:8000]",
    "plot 'stft1000.txt' u 1:2:3 with image title 'stft spectrogram'"};
